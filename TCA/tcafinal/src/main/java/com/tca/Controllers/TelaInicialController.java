...
public class TelaInicialController {
    ...
    private void VerAgendamentos() {
        ...
        try {
            // Carrega a tela de visualização dos agendamentos
            Parent root = FXMLLoader.load(getClass().getResource("/com/tca/Views/TelaConsultas.fxml"));
            Stage stage = (Stage) btnVerAgendamentos.getScene().getWindow();  // Acessa o Stage atual
            stage.setScene(new Scene(root, 600, 400));  // Altera a cena
            stage.setTitle("Ver Agendamentos");
            stage.show();  // Exibe a nova cena
        } catch (Exception e) {
            System.out.println("Erro ao trocar para a tela de agendamentos.");
            e.printStackTrace();
        }
    }
}
